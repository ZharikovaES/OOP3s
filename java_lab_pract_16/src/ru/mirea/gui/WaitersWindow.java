...
import ru.mirea.gui.parts.OrderPane;
...
public class WaitersWindow extends JFrame {
    ...
    public WaitersWindow(){
        ...
        JButton newGuest = new JButton("Обслужить гостя");
        newGuest.addActionListener(e->{
            int[] tn;
            try{
                tn = MainFrame.getInstance().getFreeNums();
            ...
            try{
                MainFrame.getInstance().addOrder(t-1, JOptionPane.showConfirmDialog(this, "Гость - совершеннолетний?", "Начало работы с клиентом", JOptionPane.YES_NO_OPTION) == 0);
